...
const Q = gql`query GetLodgementsByIngress { lodgementsByIngress { location count } }`

const COLORS = ['#4f8ef5', '#2dd4bf', '#a78bfa']
...
export default function LodgementByIngressPie() {
  const { data, loading, error } = useQuery(Q)
  if (loading) return <div className="loading">loading…</div>
  if (error) return <div className="error">error: {error.message}</div>

  const chartData = data.lodgementsByIngress.map((d, i) => ({ ...d, name: d.location, fill: COLORS[i % COLORS.length] }))
