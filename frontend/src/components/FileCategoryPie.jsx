...
const Q = gql`query GetFilesByCategory { filesByCategory { category count } }`

const COLORS = ['#4f8ef5', '#f5c542', '#3de89b', '#f04f5a']
...
export default function FileCategoryPie() {
  const { data, loading, error } = useQuery(Q)
  if (loading) return <div className="loading">loading…</div>
  if (error) return <div className="error">error: {error.message}</div>

  const chartData = data.filesByCategory.map((d, i) => ({ ...d, name: d.category, fill: COLORS[i % COLORS.length] }))
