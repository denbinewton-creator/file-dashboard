import React from 'react'
import { useQuery, gql } from '@apollo/client'
import {
  BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer,
} from 'recharts'

const Q = gql`query GetFilesByCreator { filesByCreator { creator count } }`

const CustomTooltip = ({ active, payload, label }) => {
  if (!active || !payload?.length) return null
  return (
    <div style={{ background: '#181c27', border: '1px solid #252a38', borderRadius: 8, padding: '8px 14px', fontFamily: 'DM Mono, monospace', fontSize: 12 }}>
      <div style={{ color: '#56607a', marginBottom: 4 }}>creator {label}</div>
      <div style={{ color: '#3de89b', fontWeight: 600 }}>{payload[0].value} files</div>
    </div>
  )
}

export default function FilesByCreator() {
  const { data, loading, error } = useQuery(Q)
  if (loading) return <div className="loading">loading…</div>
  if (error) return <div className="error">error: {error.message}</div>

  return (
    <ResponsiveContainer width="100%" height={280}>
      <BarChart
        data={data.filesByCreator}
        layout="vertical"
        margin={{ top: 4, right: 24, left: 4, bottom: 4 }}
      >
        <CartesianGrid strokeDasharray="3 3" stroke="#252a38" horizontal={false} />
        <XAxis
          type="number"
          tick={{ fill: '#56607a', fontSize: 10, fontFamily: 'DM Mono, monospace' }}
          tickLine={false}
          axisLine={{ stroke: '#252a38' }}
        />
        <YAxis
          dataKey="creator"
          type="category"
          tick={{ fill: '#56607a', fontSize: 11, fontFamily: 'DM Mono, monospace' }}
          tickLine={false}
          axisLine={false}
          width={60}
        />
        <Tooltip content={<CustomTooltip />} cursor={{ fill: 'rgba(255,255,255,0.04)' }} />
        <Bar dataKey="count" fill="#3de89b" radius={[0, 3, 3, 0]} maxBarSize={22} />
      </BarChart>
    </ResponsiveContainer>
  )
}
