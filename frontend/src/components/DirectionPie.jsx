...
const Q = gql`query GetFilesByDirection { filesByDirection { direction count } }`
...
export default function DirectionPie() {
  ...
  const chartData = data.filesByDirection.map((d, i) => ({ ...d, name: d.direction, fill: COLORS[i % COLORS.length] }))

  return (
    <ResponsiveContainer width="100%" height={280}>
      <PieChart>
        <Pie data={chartData} dataKey="count" nameKey="name" cx="50%" cy="50%" outerRadius={100} labelLine={false} label={renderLabel}>
          {chartData.map((entry, i) => <Cell key={i} fill={entry.fill} />)}
        </Pie>
        <Tooltip content={<CustomTooltip />} />
        <Legend formatter={(value) => <span style={{ color: '#56607a', fontSize: 12, fontFamily: 'DM Mono, monospace' }}>{value}</span>} />
      </PieChart>
    </ResponsiveContainer>
  )
}
