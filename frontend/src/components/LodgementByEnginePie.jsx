...
const Q = gql`query GetLodgementsByEngine { lodgementsByEngine { engine count } }`

const COLORS = ['#3de89b', '#4f8ef5', '#a78bfa']
...
export default function LodgementByEnginePie() {
  const { data, loading, error } = useQuery(Q)
  if (loading) return <div className="loading">loading…</div>
  if (error) return <div className="error">error: {error.message}</div>

  const chartData = data.lodgementsByEngine.map((d, i) => ({ ...d, name: d.engine, fill: COLORS[i % COLORS.length] }))
